...
import { type ConvertOptions, Converter, step } from "parser-stream/Converter"
import { type DocumentInput, type DocumentState, initialState } from "parser-stream/domain/Document"
import { CONTRACT_VERSION, isTerminal, type LiveEvent } from "parser-stream/domain/Events"
...
const make = Effect.gen(function*() {
  ...
  const create = Effect.fn("Sessions.create")(function*(input: CreateSession) {
    ...
    // That is how a per-request value such as a `Credential` reaches the

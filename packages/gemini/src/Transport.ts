...
export const stream = (request: GeminiRequest): Stream.Stream<string, HtmlStreamError, HttpClient.HttpClient> => {
  const fail = (message: string) => new HtmlStreamError({ parser: request.parser, message })

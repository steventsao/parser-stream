...
export type ConvertError = ParserError | ParserIdleError | SanitizeError | SplitError | PartLimitError
...
const make = Effect.gen(function*() {
  ...
  ): Stream.Stream<ConvertEvent, ParserError | ParserIdleError | SanitizeError> =>
    Stream.suspend(() => {
    ...
    const partStream = (index: number): Stream.Stream<ConvertEvent> =>
      Stream.unwrap(
        ...
        Stream.catchTags({
          ...
          ParserError: (error) => failed(index, error.message),
